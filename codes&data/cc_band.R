...
#load pathlist
load("/path_to_files/allbandcolon.RData")
load("/path_to_files/pathHallcolon.RData")
load("/path_to_files/pathCBcolon.RData")

#run ocean
expids<-which(rownames(expdat) %in% hallpath[[allband$pexp[r,]]]) 
cnids<-which(rownames(cndat) %in% chbdlist[[allband$pcn[r,]]])
...
#get the pathlist
save(out, file = "/path_to_files/ccbr,.RData")
